...
contract MyERC721Token is ERC721 {
    ...
    function _setTokenURI(uint256 tokenId, string memory tokenURI) internal virtual {
        // require((tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenURIs[tokenId] = tokenURI;
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        // require((tokenId), "ERC721Metadata: URI query for nonexistent token");
        return _tokenURIs[tokenId];
    }

    function transferNFT(address from, address to, uint256 tokenId) public {
        // require((msg.sender, tokenId), "You are not the owner or approved to transfer this token");
